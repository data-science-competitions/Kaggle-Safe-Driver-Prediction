...
pred_list <- foreach(i=1:nboot,
                     .errorhandling='stop', .packages=c('foreach','ROCR','randomForest'), 
                     .inorder=FALSE) %dopar% {
                         ...
                         mdl <- randomForest(formula=f, 
                                             data=X_bs, 
                                             ntree=100)
                         
                         # 3. Predict the train set
                         y_hat_bs = predict(mdl, newdata=X_bs, type="prob")
                         y_hat_bs = unlist(round(y_hat_bs[,"1"], 5))
                         stopifnot(!any(is.na(y_hat_bs)))
                         
                         # 4. Predict the test set
                         y_hat_te = predict(mdl, newdata=X_te, type="prob")
                         y_hat_te = unlist(round(y_hat_te[,2], 5))
