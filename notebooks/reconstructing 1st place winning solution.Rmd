...
for(cat_var in cat_vars){
    # 2.1 If the current variable contains less than three levels, then skip it
    if(nlevels(tidy_data[[cat_var]])<3) next
    # 2.2 Expand the factor varialbe to a set of dummy variables
    dummy_variables = model.matrix(formula(paste('~',cat_var,'-1')), tidy_data)
    # 2.3 Avoid the dummy variables trap
    dummy_variables = apply(dummy_variables,2,as.logical)[,-1] 
...
```{r feature_transformations}
num_vars = names(which(sapply(tidy_data, is.numeric)))[-1] # exclude the ID col
options(na.action='na.pass')
for(num_var in num_vars){
    # 1. Extract the current variable
    current_var = as_data_frame(tidy_data[[num_var]])
    # 2. Stabilize variance of numeric features
    # 2.1 Fit YeoJohnson power transformation
    preprocessing.power_transform = preProcess(current_var, method="YeoJohnson") 
    # 2.2 Apply YeoJohnson transformation
    current_var = predict(preprocessing.power_transform, current_var)
    # 3. Standardization
    # 3.1 Center and scale the feature distribution
    preprocessing.scale = preProcess(current_var, method=c("center","scale")) 
    # 3.2 Apply standardization
    current_var = predict(preprocessing.scale, current_var)
    # 4. Update the data set
    tidy_data[[num_var]] = unlist(current_var)
}
options(na.action='na.omit')
```
